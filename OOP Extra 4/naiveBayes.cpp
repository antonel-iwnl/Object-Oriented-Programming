...
void stats(Matrix & m, int col_num, double & mean, double & var) {
    ...
    // calculates the variance for the matrix
    if (var < 0) {
        var = 0;
    }
}
...
Matrix unique(const Matrix & m) {
    ...
    // returns the resulted matrix with unique values
    return res;
}
...
double NaiveBayes::calculate_likelihood(double mean, double var, double x) {
    //the prior probability of a class c
    // probability density function
    double eps = 1e-4; 
    double coeff = 1.0 / sqrt(2 * M_PI * var + eps);
    double expon = exp(-(pow(x - mean, 2) / (2 * var + eps))); // how likely the feature is
    return coeff*expon;
}
...
double NaiveBayes::classify(const Matrix & x) {
    // ciassifies using NaiveBayes's classification
    vector<double> posteriors(classes.getRows());
    for (int i = 0; i < classes.getRows(); i++) {
        //chooses the class with highest probability
        double & c = classes[i][0];
...
Matrix NaiveBayes::predict(Matrix & train_X) {
    // just predicts the classes for a given set based on prior results
